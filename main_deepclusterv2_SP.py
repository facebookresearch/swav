...
def train(loader, model, optimizer, epoch, schedule, local_memory_index, local_memory_embeddings):
    ...
    assignments_crop_beta, emb_beta, centroids_beta = cluster_memory(model, local_memory_index, local_memory_embeddings, len(loader.dataset))         
    ...
    for it, (idx, inputs) in enumerate(loader):
        """
        print("inputs", inputs)
        print("inputs0_size", inputs[0].size())
        print("idx", idx)
        """
        ...
        for h in range(len(args.nmb_prototypes)):
            ...
            targets = assignments_crop_beta[h][idx].repeat(sum(args.nmb_crops)).cuda(non_blocking=True)             # targets €(20)                                                                                                                                                                                           
...
def cluster_memory(model, local_memory_index, local_memory_embeddings, size_dataset, nmb_kmeans_iters=10):
    l = 500                                                                                                # number of NN in similarity graph                                                                                                    
    n = local_memory_embeddings.size(dim=1)                                                                # 980 (number of features/pictures)
    j = 0                                                                                                   
    assignments = -100 * torch.ones(len(args.nmb_prototypes), size_dataset).long()                                                                                               # crop_beta no backprob
    
    with torch.no_grad():
        for i_K, K in enumerate(args.nmb_prototypes):
            # SPECTRAL CLUSTERING

            # cos similarty matrix S
            S = torch.mm(local_memory_embeddings[j], torch.transpose(local_memory_embeddings[j], 0, 1))    # S (980x980) cos-similarity between each feature 
                
            # weight matrix (l-NN graph) 
            W = torch.zeros((n,n)).cuda(non_blocking=True)
            v, ind = torch.topk(S, l)                                                                      # l biggest entries (WITH 1) of S & their indices
            ...
            # sym_Laplacian
            L = torch.zeros((n,n)).cuda(non_blocking=True)
            ...
            L = torch.mm(torch.mm(D_MP,(D-W)),D_MP)
                
            # EV
            EW,EV = torch.eig(L,eigenvectors=True)       
            First_k_EVs = torch.zeros((n,K))
            for x in range(K):
                First_k_EVs[:,x] = EV[:,x] + i*EV[:,x+1]                                              # First K EVs (real Numbers)
            ...
            centroids = torch.empty(K, K).cuda(non_blocking=True)
            if args.rank == 0:
                random_idx = torch.randperm(len(T))[:K]
                assert len(random_idx) >= K, "please reduce the number of centroids"
                centroids = T[random_idx]
            dist.broadcast(centroids, 0)

            for n_iter in range(nmb_kmeans_iters + 1):

                # E step
                dot_products = torch.cdist(T, centroids.t(), p=2)                                           # squared euclidian distance
                ...
                emb_sums = torch.zeros(K, K).cuda(non_blocking=True)
            ...
            j = (j + 1) % len(args.crops_for_assign)                                                            # crops_for_assign=[0,1] --> len()=2
    return assignments, T, centroids                                                                    # assignments €(987); T €(980x8); centroids €(8x8)
